<?php
    ...
    if ($verb == "GET") {
		...
		$rs = mysqli_query($link,"SELECT l.id,l.employee_id, l.start_date, l.leave_days_awarded, l.reason, e.employee_name FROM employee_leave_days l, employees e WHERE e.id = l.employee_id AND l.employee_id = '$e_id' AND l.type = 2 ORDER BY l.id DESC");
	...
	if ($verb == "PUT") {
        ...
        $no_of_days = mysql_real_escape_string($request_vars["leave_days_awarded"]);
		...
		$reason = mysql_real_escape_string($request_vars["reason"]);					
			// INSERT COMMAND 
		$insert_query = "INSERT INTO employee_leave_days(employee_id,leave_days_awarded,start_date, type, reason) VALUES ('".$employee_id."','".$no_of_days."','".$start_date."',2,'".$reason."')";
