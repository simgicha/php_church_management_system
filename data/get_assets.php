<?php
    ...
    if ($verb == "GET") {
		...
		$rs = mysqli_query($link,"SELECT a.id, a.item_name, a.local_church_id, l.name, a.purchase_date, a.initial_value, a.item_tag, a.disposal_date, a.disposal_amount, a.status, a.quantity, o.owner_name, o.id as owner_id, a.item_category_id, c.category_name FROM assets a, local_church l, assets_owners o, asset_categories c WHERE a.local_church_id = l.id AND o.id = a.owner_id AND a.type = 1 AND c.id = a.item_category_id ORDER BY id DESC");
	...
	if ($verb == "PUT") {
			...
			$item_category_id = mysql_real_escape_string($request_vars["item_category_id"]);
			$status = mysql_real_escape_string($request_vars["status"]);				
			$purchase_date = date('Y-m-d', strtotime( $purchase_date1));
			// INSERT COMMAND 
			$insert_query = "INSERT INTO assets(item_name,local_church_id, purchase_date,initial_value, item_tag, quantity, owner_id, type, item_category_id, status) VALUES ('".$item_name."','".$local_church_id."','".$purchase_date."','".$initial_value."','".$item_tag."','".$quantity."','".$owner_id."',1,'".$item_category_id."', '".$status."')";
	...
	if ($verb == "POST") {
			...
			$purchase_date = date('Y-m-d', strtotime( $purchase_date1));			
			$item_category_id = mysql_real_escape_string($_POST["item_category_id"]);
			$status = mysql_real_escape_string($_POST["status"]);				
			$id = mysql_real_escape_string($_POST["id"]);
			
			$rs = mysqli_query($link,"UPDATE assets SET item_name = '" .$item_name ."',local_church_id = '" .$local_church_id ."',purchase_date = '" .$purchase_date ."',initial_value = '" .$initial_value."',quantity = '" .$quantity."',item_tag = '" .$item_tag."', item_category_id ='".$item_category_id."',status = '" .$status."' WHERE id = " .$id);
